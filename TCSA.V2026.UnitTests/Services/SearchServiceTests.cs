...
public class SearchServiceTests
{
    ...
    [TestCase("csharp", "c#")]
    [TestCase("aspnet", "asp.net")]
    [TestCase("fsharp", "f#")]
    [TestCase("dotnet", ".net")]
    public void GetHighlightTerms_ExpandsReverseAlias(string query, string expectedTerm)
    {
        var sut = new SearchService([]);

        var terms = sut.GetHighlightTerms(query);

        Assert.That(terms, Contains.Item(expectedTerm));
    }

    [Test]
    public void GetHighlightTerms_ReturnsTokenizedTerms()
    {
        var sut = new SearchService([]);

        var terms = sut.GetHighlightTerms("calculator intro");

        Assert.That(terms, Contains.Item("calculator"));
        Assert.That(terms, Contains.Item("intro"));
    }

    [Test]
    public void GetHighlightTerms_IsCaseInsensitive()
    {
        var sut = new SearchService([]);

        var terms = sut.GetHighlightTerms("Calculator");

        Assert.That(terms.Contains("CALCULATOR"), Is.True);
    }

    [Test]
    public void GetHighlightTerms_ReturnsEmpty_WhenAllTokensAreSingleChar()
    {
        var sut = new SearchService([]);

        var terms = sut.GetHighlightTerms("a b");

        Assert.That(terms, Is.Empty);
    }

    [Test]
    public void GetHighlightTerms_ExcludesSingleCharTokens()
    {
        var sut = new SearchService([]);

        var terms = sut.GetHighlightTerms("a design");

        Assert.That(terms, Does.Not.Contain("a"));
        Assert.That(terms, Contains.Item("design"));
    }
}
